...
#import <Window/OSX/NSImage+raw.h>
#import <Window/OSX/Scaling.h>
#import <Window/OSX/SFApplication.h>
#import <Window/OSX/SFOpenGLView.h>
#import <Window/OSX/SFWindow.h>
#import <Window/OSX/SFWindowController.h>
...
-(id)initWithWindow:(NSWindow*)window
{
    if ((self = [super init]))
    {
        ...
        if (m_window == nil)
        {
            tinySFML::err() << "No window was given to -[SFWindowController initWithWindow:]." << std::endl;
            return self;
        }
        ...
        if (m_oglView == nil)
        {
            tinySFML::err() << "Could not create an instance of NSOpenGLView "
...
////////////////////////////////////////////////////////
-(id)initWithMode:(const tinySFML::VideoMode&)mode andStyle:(unsigned long)style
{
    ...
    if ([NSThread currentThread] != [NSThread mainThread])
    {
         ...
         */
        tinySFML::err() << "Cannot create a window from a worker thread. (OS X limitation)" << std::endl;
    ...
    if ((self = [super init]))
    {
        ...
        m_oglView = nil;
        m_requester = 0;
        m_fullscreen = (style & tinySFML::Style::Fullscreen);
        m_restoreResize = NO;
...
////////////////////////////////////////////////////////
-(void)setupFullscreenViewWithMode:(const tinySFML::VideoMode&)mode
{
    // Create a screen-sized window on the main display
    tinySFML::VideoMode desktop = tinySFML::VideoMode::getDesktopMode();
    tinySFML::priv::scaleInWidthHeight(desktop, nil);
    ...
    if (m_window == nil)
    {
        tinySFML::err() << "Could not create an instance of NSWindow "
    ...
    if (masterView == nil)
    {
        tinySFML::err() << "Could not create an instance of SFBlackView "
                  << "in -[SFWindowController setupFullscreenViewWithMode:]."
                  << std::endl;
        return;
    }
...
////////////////////////////////////////////////////////
-(void)setupWindowWithMode:(const tinySFML::VideoMode&)mode andStyle:(unsigned long)style
{
    // We know that style & tinySFML::Style::Fullscreen is false.
    ...
    if (style & tinySFML::Style::Titlebar)
        nsStyle |= NSTitledWindowMask | NSMiniaturizableWindowMask;
    if (style & tinySFML::Style::Resize)
        nsStyle |= NSResizableWindowMask;
    if (style & tinySFML::Style::Close)
...
////////////////////////////////////////////////////////
-(void)setRequesterTo:(tinySFML::priv::WindowImplCocoa*)requester
{
    // Forward to the view.
...
////////////////////////////////////////////////////////
-(tinySFML::WindowHandle)getSystemHandle
{
    return m_window;
}
...
-(void)setIconTo:(unsigned int)width
            ...
            with:(const tinySFML::Uint8*)pixels
{
...
-(void)processEvent
{
    ...
    if ([NSThread currentThread] != [NSThread mainThread])
    {
         ...
         * for more information.
         */
        tinySFML::err() << "Cannot fetch event from a worker thread. (OS X restriction)" << std::endl;
