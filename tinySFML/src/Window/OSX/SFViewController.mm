...
#import <Window/OSX/SFViewController.h>
...
-(id)initWithView:(NSView *)view
{
    if ((self = [super init]))
    {
        ...
        if (m_view == nil)
        {
            tinySFML::err() << "No view was given to initWithWindow:." << std::endl;
            return self;
        }
...
-(void)setWindowPositionToX:(int)x Y:(int)y
{
    (void)x;
    (void)y;
    tinySFML::err() << "Cannot move SFML area when SFML is integrated in a NSView. Use the view handler directly instead." << std::endl;
}
...
{
    (void)title;
    tinySFML::err() << "Cannot change the title of the SFML area when SFML is integrated in a NSView." << std::endl;
}
...
-(void)closeWindow
{
    tinySFML::err() << "Cannot close SFML area when SFML is integrated in a NSView." << std::endl;
}
            ...
            with:(const tinySFML::Uint8*)pixels
{
    ...
    (void)height;
    (void)pixels;
    tinySFML::err() << "Cannot set an icon when SFML is integrated in a NSView." << std::endl;
}
