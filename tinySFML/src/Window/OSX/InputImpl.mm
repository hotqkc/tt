...
SFOpenGLView* getSFOpenGLViewFromSFMLWindow(const WindowBase& window)
{
    ...
    if ([nsHandle isKindOfClass:[NSWindow class]])
    {
        ...
        if (![view isKindOfClass:[SFOpenGLView class]])
        {
            ...
            {
                tinySFML::err() << "The content view is not a valid SFOpenGLView"
                          << std::endl;
    ...
    else if ([nsHandle isKindOfClass:[NSView class]])
    {
        ...
        // No matching subview ?
        if (view == nil)
            tinySFML::err() << "Cannot find a valid SFOpenGLView subview." << std::endl;
    }
    else
    {
        if (nsHandle != 0)
            tinySFML::err() << "The system handle is neither a <NSWindow*> nor <NSView*>"
...
Vector2i InputImpl::getMousePosition()
{
    ...
    pos.y = tinySFML::VideoMode::getDesktopMode().height - pos.y;
...
void InputImpl::setMousePosition(const Vector2i& position, const WindowBase& relativeTo)
{
    ...
    setMousePosition(tinySFML::Vector2i(p.x, p.y) * scale);
}
